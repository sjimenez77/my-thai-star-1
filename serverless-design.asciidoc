...
In order to be compatible with the other backend implementations, we must follow the https://github.com/oasp/oasp4j/wiki/guide-rest[naming conventions] proposed for _Oasp4j_ applications. We will define the following _end points_ for the listed services.
